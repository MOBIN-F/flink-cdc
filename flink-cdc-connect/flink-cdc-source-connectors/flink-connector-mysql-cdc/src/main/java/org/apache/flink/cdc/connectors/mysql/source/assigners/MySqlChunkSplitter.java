...
public class MySqlChunkSplitter implements ChunkSplitter {
        ...
        try {
            ...
            splitType =
                    ChunkUtils.getChunkKeyColumnType(
                            splitColumn, sourceConfig.isCastTinyIntToInt());
            minMaxOfSplitColumn =
    ...
    private int getDynamicChunkSize(
            ...
            int chunkSize,
            long approximateRowCnt) {
        if (!isEvenlySplitColumn(splitColumn, sourceConfig.isCastTinyIntToInt())) {
            return -1;
        }
    ...
    private static boolean isEvenlySplitColumn(Column splitColumn, boolean isCastTinyIntToInt) {
        DataType flinkType = MySqlTypeUtils.fromDbzColumn(splitColumn, isCastTinyIntToInt);

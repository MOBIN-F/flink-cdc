...
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1012 -> Eve, 1012,
// Eve, 17, 0, 1024144, age < 20], op=INSERT, meta=()}",
//                        "AlterColumnTypeEvent{tableId=%s.members, typeMapping={age=DOUBLE},
// oldTypeMapping={age=INT}}",
...
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1013 -> Fiona,
// 1013, Fiona, 16.0, 1026169, age < 20], op=INSERT, meta=()}",
//                        "TruncateTableEvent{tableId=%s.members}",
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1014 -> Gem, 1014,
// Gem, 17.0, 1028196, age < 20], op=INSERT, meta=()}",
...
//                false,
//                Arrays.asList(
//                        "AddColumnEvent{tableId=%s.merged,
// addedColumns=[ColumnWithPosition{column=`gender` TINYINT, position=AFTER,
// existedColumnName=tag}]}",
//                        "DataChangeEvent{tableId=%s.merged, before=[], after=[1012 -> Eve, 1012,
// Eve, 17, 1024144, age < 20, 0], op=INSERT, meta=()}",
//                        "AlterColumnTypeEvent{tableId=%s.merged, typeMapping={age=DOUBLE},
// oldTypeMapping={age=INT}}",
//                        "AddColumnEvent{tableId=%s.merged,
// addedColumns=[ColumnWithPosition{column=`biological_sex` TINYINT, position=AFTER,
// existedColumnName=gender}]}",
//                        "DataChangeEvent{tableId=%s.merged, before=[], after=[1013 -> Fiona, 1013,
// Fiona, 16.0, 1026169, age < 20, null, null], op=INSERT, meta=()}",
//                        "DataChangeEvent{tableId=%s.merged, before=[], after=[1014 -> Gem, 1014,
// Gem, 17.0, 1028196, age < 20, null, null], op=INSERT, meta=()}"));
//    }
//
//    @Test
//    @Disabled
...
//                Arrays.asList(
//                        "An exception was triggered from Schema change applying task. Job will
// fail now.",
//                        "org.apache.flink.util.FlinkRuntimeException: Failed to apply schema
// change event.",
//                        "Caused by:
...
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1012 -> Eve, 1012,
// Eve, 17, 1024144, age < 20], op=INSERT, meta=()}",
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1013 -> Fiona,
// 1013, Fiona, null, 1026169, age < 20], op=INSERT, meta=()}",
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1014 -> Gem, 1014,
// Gem, null, 1028196, age < 20], op=INSERT, meta=()}"),
//                Arrays.asList(
//                        "Failed to apply schema change AddColumnEvent{tableId=%s.members,
// addedColumns=[ColumnWithPosition{column=`gender` TINYINT, position=AFTER,
// existedColumnName=age}]}, but keeps running in tolerant mode. Caused by:
...
//                        "Failed to apply schema change AlterColumnTypeEvent{tableId=%s.members,
// typeMapping={age=DOUBLE}, oldTypeMapping={age=INT}}, but keeps running in tolerant mode. Caused
// by: UnsupportedSchemaChangeEventException{applyingEvent=AlterColumnTypeEvent{tableId=%s.members,
// typeMapping={age=DOUBLE}, oldTypeMapping={age=INT}}, exceptionMessage='Rejected schema change
// event since error.on.schema.change is enabled.', cause='null'}",
//                        "Failed to apply schema change RenameColumnEvent{tableId=%s.members,
// nameMapping={gender=biological_sex}}, but keeps running in tolerant mode. Caused by:
// UnsupportedSchemaChangeEventException{applyingEvent=RenameColumnEvent{tableId=%s.members,
// nameMapping={gender=biological_sex}}, exceptionMessage='Rejected schema change event since
// error.on.schema.change is enabled.', cause='null'}",
//                        "Failed to apply schema change DropColumnEvent{tableId=%s.members,
// droppedColumnNames=[biological_sex]}, but keeps running in tolerant mode. Caused by:
// UnsupportedSchemaChangeEventException{applyingEvent=DropColumnEvent{tableId=%s.members,
// droppedColumnNames=[biological_sex]}, exceptionMessage='Rejected schema change event since
// error.on.schema.change is enabled.', cause='null'}",
//                        "Failed to apply schema change TruncateTableEvent{tableId=%s.members}, but
// keeps running in tolerant mode. Caused by:
// UnsupportedSchemaChangeEventException{applyingEvent=TruncateTableEvent{tableId=%s.members},
// exceptionMessage='Rejected schema change event since error.on.schema.change is enabled.',
// cause='null'}",
//                        "Failed to apply schema change DropTableEvent{tableId=%s.members}, but
// keeps running in tolerant mode. Caused by:
// UnsupportedSchemaChangeEventException{applyingEvent=DropTableEvent{tableId=%s.members},
// exceptionMessage='Rejected schema change event since error.on.schema.change is enabled.',
// cause='null'}"));
//    }
//
...
// oldTypeMapping={age=INT}}",
//                        "AddColumnEvent{tableId=%s.members,
// addedColumns=[ColumnWithPosition{column=`biological_sex` TINYINT, position=LAST,
// existedColumnName=null}]}",
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1013 -> Fiona,
// 1013, Fiona, 16.0, 1026169, age < 20, null, null], op=INSERT, meta=()}",
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1014 -> Gem, 1014,
// Gem, 17.0, 1028196, age < 20, null, null], op=INSERT, meta=()}"));
//    }
//
...
// nameMapping={gender=biological_sex}}",
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1013 -> Fiona,
// 1013, Fiona, 16.0, null, 1026169, age < 20], op=INSERT, meta=()}",
//                        "TruncateTableEvent{tableId=%s.members}",
//                        "DataChangeEvent{tableId=%s.members, before=[], after=[1014 -> Gem, 1014,
// Gem, 17.0, null, 1028196, age < 20], op=INSERT, meta=()}"),
//                Arrays.asList(
//                        "Ignored schema change DropColumnEvent{tableId=%s.members,
// droppedColumnNames=[biological_sex]}.",
...
//                                + "transform:\n"
//                                + "  - source-table: %s.\\.*\n"
//                                + "    projection: CAST(id AS VARCHAR) || ' -> ' || name AS uid,
// *, id * id AS id_square, 'age < 20' as tag\n"
//                                + "    filter: age < 20\n"
//                                + "  - source-table: %s.\\.*\n"
//                                + "    projection: CAST(id AS VARCHAR) || ' -> ' || name AS uid,
// *, 0 - id * id AS id_square, 'age >= 20' as tag\n"
//                                + "    filter: age >= 20\n"
//                                + (mergeTable
//                                        ? String.format(
//                                                "route:\n"
...
//        String mysqlJdbcUrl =
//                String.format(
//                        "jdbc:mysql://%s:%s/%s", MYSQL.getHost(), MYSQL.getDatabasePort(),
// dbName);
//
//        try (Connection conn =
...
//                Stream.of(
//                                "CreateTableEvent{tableId=%s.%s, schema=columns={`uid` STRING,`id`
// INT NOT NULL,`name` VARCHAR(17),`age` INT,`id_square` INT,`tag` STRING}, primaryKeys=id,
// options=()}",
//                                "DataChangeEvent{tableId=%s.%s, before=[], after=[1009 -> Bob,
// 1009, Bob, 20, -1018081, age >= 20], op=INSERT, meta=()}",
//                                "DataChangeEvent{tableId=%s.%s, before=[], after=[1008 -> Alice,
// 1008, Alice, 21, -1016064, age >= 20], op=INSERT, meta=()}",
//                                "DataChangeEvent{tableId=%s.%s, before=[], after=[1011 -> Derrida,
// 1011, Derrida, 18, 1022121, age < 20], op=INSERT, meta=()}",
//                                "DataChangeEvent{tableId=%s.%s, before=[], after=[1010 -> Carol,
// 1010, Carol, 19, 1020100, age < 20], op=INSERT, meta=()}")
...
//                String.format(
//                        "DataChangeEvent{tableId=%s.%s, before=[], after=[0 -> __fence__, 0,
// __fence__, 0, 0, age < 20], op=INSERT, meta=()}",
//                        dbName, tableName),
//                taskManagerConsumer);
//    }
//
...
// SchemaEvolvingTransformE2eITCase.EVENT_WAITING_TIMEOUT;
